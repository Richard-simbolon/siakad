...
class Periode extends Controller
            ...
            "title"=>["type"=>"null" , "value"=>"null" , "validation" => ""] ,
                ...
                static $tablename = "Periode";
                public function index()
                {
                    $data = PeriodeModel::get();
                    $title = ucfirst(request()->segment(1))." ".ucfirst(request()->segment(2));
                    $tableid = "Periode";
                    $table_display = DB::getSchemaBuilder()->getColumnListing("master_periode");
                ...
                public function sinc(){
                    ...
                    //echo $token; exit;
                    $data = array('act'=>"GetPeriode" , "token"=>$token, "filter"=> "","limit"=>"" , "offset" =>0);
                        ...
                        //if(count($result['data']) > 1){
                            ...
                            try{
                                foreach($result['data'] as $item){
                                    PeriodeModel::updateOrInsert(array('id_prodi'=> $item['id_prodi']) , array('id_prodi'=> $item['id_prodi'] , 'periode_pelaporan'=>$item['periode_pelaporan']));
                                }
                                DB::commit();
                                DB::table('sinkronisasi_logs')
                                ->insert(array('title' => 'GetPeriode' ,'created_by'=> Auth::user()->id ,'created_at'=>date('Y-m-d H:i:s')));
                ...
                public function create(){
                    ...
                    $table = array_diff(DB::getSchemaBuilder()->getColumnListing("master_periode"), static::$exclude);
                    $exclude = static::$exclude;
                    $Tableshow = static::$Tableshow;
                    $html = static::$html;
                    $column = 1;
                ...
                public function save(Request $request){
                    ...
                    $field = [];
                    $data = [];
                    $table = DB::getSchemaBuilder()->getColumnListing("master_periode");
                    ...
                    $save  = PeriodeModel::firstOrCreate($data);
                    if($save){
                ...
                public function paging(Request $request){
                    return Datatables::of(PeriodeModel::all())->make(true);
                }
